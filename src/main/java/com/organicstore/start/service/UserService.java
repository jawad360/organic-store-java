...
public class UserService {
	...
	public boolean validateUserDocument(User user) {
		if (user.getId() == null || user.getId() == "" || user.getName() == null || user.getName() == ""
				|| user.getEmail() == null || user.getEmail() == "" || user.getPassword() == null
				|| user.getPassword() == "" || user.getPhone() == null || user.getPhone() == "")
			return false;
		return true;
	}
	...
	public boolean validateOrderDocument(Order order) {
		if (order.getId() == null || order.getId() == "")
			for (OrderDetails orderDetails : order.getOrderDetails())
				if (orderDetails.getInventoryId() == null || orderDetails.getInventoryId() == ""
						|| order.getPrice() == 0.0)
					return false;
		return true;
	}
	...
	public void updateUser(User user) {
		if (user.getId() != null && user.getId() != "") {
			...
			if (isPresent) {
				...
				// if((newUser.getAddress() != null && newUser.getAddress().getPincode() != null
				// && newUser.getAddress().getPincode() != ""))
	...
	public void deleteUser(String id) throws ResourceNotFoundException {
		...
		if (user.isPresent()) {
			...
			String message = messageSource.getMessage("user_not_found", null, "Default", new Locale("en"));
